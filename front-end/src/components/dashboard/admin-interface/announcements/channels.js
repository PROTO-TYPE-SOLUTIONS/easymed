...
import { createAnnouncementChannels } from '@/redux/service/announcements';
...
const Channels = () => {
    ...
    const { channels }  = useSelector((store)=> store.announcement);
    ...
    const addNewChannel = async (values) => {
        setLoading(true)
        try{
            await createAnnouncementChannels(auth, values)
            setLoading(false)
  ...
  return (
    <div>
        <h2 className='my-10 font-bold text-xl'>Add Channels</h2>
        <Formik
      ...
      <EditChannelModal open={editOpen} setOpen={setEditOpen} selectedRowData={selectedRowData}  />
        </div>
    </div>
  )
}
