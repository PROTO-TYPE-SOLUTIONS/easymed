...
const ApproveResults = ({ selectedData, approveOpen, setApproveOpen }) => {
    ...
    useEffect(()=> {
        if(auth){
            ...
            if(selectedData && selectedData.category === "quantitative"){
                dispatch(getAllResultPanelsByResult(selectedData?.id, auth))
            }else{
                dispatch(getAllQualitativeResultPanelsByResult(selectedData?.id, auth))
            }
        }
    },[])
        ...
        >
            <DialogTitle id="alert-dialog-title">
                <div className='flex items-center min-h-6 justify-between px-2 bg-yellow-200 py-2'>
                    <p className='font-semibold'>{` Approve these results? `}</p>
                </div>
            </DialogTitle>
            <DialogContent>
            <Formik
