...
export default async function handler(req, res) {
    if (req.method === API_METHODS.GET) {
        try {
            ...
            const response = await backendAxiosInstance.get(`${API_URL.SALE_BY_DATE_RANGE_AND_ITEM_PDF}`, config);

            // Save the PDF file to the public directory
            const filePath = path.join(process.cwd(), 'public', `sale_by_date_range_and_item.pdf`);
            console.log("PATH",filePath)
            await fs.writeFile(filePath, response.data, 'binary');
            

            // Send the link to the PDF file as a response to the client
            const publicURL = `/sale_by_date_range_and_item.pdf`;
