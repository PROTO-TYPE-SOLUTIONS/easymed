...
export default async function handler(req, res) {
    if (req.method === API_METHODS.GET) {
        ...
        try {
            ...
            await backendAxiosInstance.post(`${API_URL.FETCH_INSURANCE}`, body, config)
                .then(response => {
                    res.status(200).json(response.data);
                })
                .catch(e => {
        ...
        try {
            ...
            await backendAxiosInstance.patch(`${API_URL.FETCH_INSURANCE}${query.insurance_id}/`, body, config).then(response => {
                res.status(200).json(response.data);
